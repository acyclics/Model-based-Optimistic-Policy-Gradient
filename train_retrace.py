...
from mbopg.mbrl_retrace import MBRL_solver
...
def anneal_k(initial_k, final_k, timestep, final_timestep):
    decay = (final_k / initial_k) ** (1.0 / final_timestep)
    return initial_k * decay ** timestep

for loop_n in range(N_OVERALL_LOOPS):
    mbrl_solver.reset_noise()
    mbrl_solver.k = int(anneal_k(2, 16, loop_n, N_OVERALL_LOOPS))
    policy, critic = mbrl_solver.solve(network, dataset_state, dataset_action, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob, reset=False, verbose=True)
