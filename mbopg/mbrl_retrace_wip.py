...
class MBRL_solver(nn.Module):
    ...
    def _compute_retrace(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob):
        ...
        rollout = self._rollout(network, obs[:, 0])
        states, actions, act_log_probs, noise_log_probs, total_log_probs, rewards, dones, next_obs, next_action, next_act_log_prob, next_noise, next_noise_log_prob = rollout
        
        initial_obs = torch.cat([initial_obs, states.unsqueeze(dim=1)], dim=1)
        next_states = torch.cat()
        initial_obs2 = torch.cat([initial_obs2, ])

        del_q = 0.0
        mask = torch.zeros((initial_dones.shape[0], 1), device=self.device, dtype=torch.float32)

        for s in range(self.k - 1 + self.horizon):
            discount = self.gamma ** s
            c = 1.0
            for i in range(1, s):
                task_dist, _ = self.actor(initial_obs[:, i])
                task_logprob = task_dist.log_prob(initial_act[:, i]).sum(-1, keepdim=True)
                c *= torch.min(torch.ones_like(task_logprob), task_logprob / initial_logprob[:, i])
            r = initial_rewards[:, s]
            o1 = initial_obs[:, s]
            a1 = initial_act[:, s]
            lp1 = initial_logprob[:, s]
            o2 = initial_obs[:, s+1]
            act_dist, noise_dist = self.actor(o2)
            a = act_dist.sample()
            lp = act_dist.log_prob(a).sum(-1, keepdim=True)
            nextQ1, nextQ2 = self.critic_target(o2, a)
            nextQ = torch.min(nextQ1, nextQ2) - self.alpha.detach() * lp
            oldQ1, oldQ2 = self.critic_target(o1, a1)
            oldQ = torch.min(oldQ1, oldQ2)
            del_q += (1.0 - mask.float()) * discount * c * (r + self.gamma * nextQ - oldQ)
            mask = torch.logical_or(mask, initial_dones[:, s])
        
        target_Q1, target_Q2 = self.critic_target(initial_obs[:, 0], initial_act[:, 0])
        target_Q = torch.min(target_Q1, target_Q1) + del_q
        target_Q = target_Q.detach()
                
        current_Q1, current_Q2 = self.critic(initial_obs[:, 0], initial_act[:, 0])
        err1 = (current_Q1 - target_Q).pow(2)
        err2 = (current_Q2 - target_Q).pow(2)
    ...
    def _solve(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob, verbose=True):
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % self.target_update_frequency == 0:
                ...
                if verbose:
                    print(f"Iteration {epoch} ; Actor value = {-actor_loss} ; Critic loss = {critic_loss} ; Retrace loss = {retrace_loss}")
                    sys.stdout.flush()
