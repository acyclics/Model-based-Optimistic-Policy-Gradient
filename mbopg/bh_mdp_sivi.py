...
class CombinedHyperNetwork(nn.Module):
    ...
    def __init__(self, noise_dim, obs_units, act_units, out_units, hidden_units, hidden_layers, initial_logvar, h_units, device):
        ...
        self.hypernet_obs = SingleHyperNetwork(1, obs_units, hidden_units, h_units, device)
        self.hypernets.append(self.hypernet_obs)

        self.hypernet_act = SingleHyperNetwork(1, act_units, hidden_units, h_units, device)
        self.hypernets.append(self.hypernet_act)

        self.hypernet_tgt = SingleHyperNetwork(1, hidden_units + hidden_units, hidden_units, h_units, device)
        self.hypernets.append(self.hypernet_tgt)
        
        for i in range(hidden_layers):
            hypernet_hidden = SingleHyperNetwork(1, hidden_units, hidden_units, h_units, device)
            self.hypernet_hidden.append(hypernet_hidden)
            self.hypernets.append(hypernet_hidden)
        
        self.hypernet_state = SingleHyperNetwork(1, hidden_units, out_units, h_units, device)
        #self.hypernet_state_var = SingleHyperNetwork(noise_dim, hidden_units, out_units, device)

        self.hypernet_reward = SingleHyperNetwork(1, hidden_units, 1, h_units, device)
        #self.hypernet_reward_var = SingleHyperNetwork(noise_dim, hidden_units, 1, device)

        self.hypernet_done = SingleHyperNetwork(1, hidden_units, 1, h_units, device)
    ...
    def get_all_weights(self, num_samples, z=None):
        ...
        mu_obs = self.hypernet_obs(z[:, 0])
        all_mu.append(mu_obs)

        mu_act = self.hypernet_act(z[:, 0])
        all_mu.append(mu_act)

        mu_tgt = self.hypernet_tgt(z[:, 0])
        all_mu.append(mu_tgt)

        for i in range(self.hidden_layers):
            mu = self.hypernet_hidden[i](z[:, 1])
            all_mu.append(mu)
        
        mu_state = self.hypernet_state(z[:, 2])
        #mu_state_var = self.hypernet_state_var(z)

        mu_reward = self.hypernet_reward(z[:, 3])
        #mu_reward_var = self.hypernet_reward_var(z)

        mu_done = self.hypernet_done(z[:, 4])
...
class Multi_SI_BayesNetwork(nn.Module):
    ...
    def calculate_hypernet_loss(self, hypernet, z_J, z_K, logvar, idx):
        K = self.K + 1
    ...
    def calculate_head_loss(self, combinedhypernetwork):
        ...
        for idx, hypernet in enumerate(combinedhypernetwork.hypernets):
            if idx >=0 or idx <=2:
                idx = 0
            elif idx >= 10 + 3:
                idx = idx - 10 + 3 + 2
                ...
                idx = 1
            w, w_inner, mu_star_inner = self.calculate_hypernet_loss(hypernet, z_J, z_K, logvar, idx)
