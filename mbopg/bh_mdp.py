...
class HypernetWeight(nn.Module):
    def __init__(self, shape, is_hyper_w, noise_shape=1, activation=nn.LeakyReLU()):
...
class SingleHyperNetwork(nn.Module):
    ...
    def __init__(self, noise_dim, in_units, out_units, device):
        super(SingleHyperNetwork, self).__init__()
        self.noise_dim = noise_dim
        self.out_units = out_units
        self.in_units = in_units

        self.mu_w = HypernetWeight((out_units, in_units), True, noise_shape=noise_dim)
...
class CombinedHyperNetwork(nn.Module):
    ...
    def __init__(self, noise_dim, obs_units, act_units, out_units, hidden_units, hidden_layers, initial_logvar, device):
        ...
        self.hypernet_obs = SingleHyperNetwork(noise_dim, obs_units, hidden_units // 2, device)
        self.hypernets.append(self.hypernet_obs)

        self.hypernet_act = SingleHyperNetwork(noise_dim, act_units, hidden_units // 2, device)
        self.hypernets.append(self.hypernet_act)
        
        for i in range(hidden_layers):
            hypernet_hidden = SingleHyperNetwork(noise_dim, hidden_units, hidden_units, device)
            self.hypernet_hidden.append(hypernet_hidden)
            self.hypernets.append(hypernet_hidden)
        
        self.hypernet_state = SingleHyperNetwork(noise_dim, hidden_units, out_units, device)
        self.hypernet_reward = SingleHyperNetwork(noise_dim, hidden_units, 1, device)
        self.hypernet_done = SingleHyperNetwork(noise_dim, hidden_units, 1, device)
...
class PrimaryNetwork(nn.Module):
    ...
    def feedforward(self, obs, action, w):
        ...
        for i in range(w.shape[0]):
            o = F.linear(obs, w_obs[i], self.obs_bias)
            a = F.linear(action, w_act[i], self.act_bias)

            embed = torch.cat([o, a], dim=-1)
            embed = torch.tanh(embed)

            y = embed
            for j in range(self.hidden_layers):
                y = F.linear(y, all_ws[j][i], self.h_bias[j])
                y = torch.tanh(y)
            
            next_state = F.linear(y + embed, w_state[i], self.state_bias)
            reward = F.linear(y + embed, w_reward[i], self.reward_bias)
            done = F.linear(y + embed, w_done[i], self.done_bias)

            next_states.append(next_state)
            rewards.append(reward)
            dones.append(done)

        next_states = torch.stack(next_states, dim=1)# + obs_unsqueezed
...
class Multi_SI_BayesNetwork(nn.Module):
    ...
    def __init__(self, obs_units, act_units, obs_scalings, hidden_units=64,
                 hidden_layers=3, LLH_var=1.0, state_LLH_var=0.1, initial_logvar=-35.0,
                 lr=1e-4, grad_norm=5.0, noise_dim=2, device='cpu'):
        ...
        self.combinedhypernetwork = CombinedHyperNetwork(self.noise_dim, obs_units, act_units, obs_units, hidden_units, hidden_layers, initial_logvar, device)

        self.params = self.combinedhypernetwork.get_all_params() + list([self.combinedhypernetwork.logvar]) + list(self.primarynet.parameters())
    ...
    def train(self, state, action, rewards, dones, next_states):
        ...
        ss_log_lik = -0.5 * torch.sum((1.0 - dones) * (next_states - ss_mean) ** 2 / self.state_LLH_var, 2)
