...
class MBRL_solver(nn.Module):
    ...
    def _get_retrace(self, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob):
        with torch.no_grad():
            initial_obs1 = []
            initial_obs2 = []
            initial_act = []
            ...
            for _ in range(self.actor_iterations_per_epoch):
                ridx = np.random.randint(0, len(dataset_next_states) - self.k)
                ...
                for kdx in range(self.k):
                    init_obs1 = dataset_states[ridx + kdx]
                    init_obs2 = dataset_next_states[ridx + kdx]
                    act = dataset_actions[ridx + kdx]
                    rew = dataset_rewards[ridx + kdx]
                    d = dataset_dones[ridx + kdx]
                    lp = dataset_logprob[ridx + kdx]

                    init_obs1s.append(init_obs1)
                    init_obs2s.append(init_obs2)
                    acts.append(act)
                    rews.append(rew)
                    ds.append(d)
                    lps.append(lp)
                
                init_obs1s = torch.stack(init_obs1s, dim=0)
                init_obs2s = torch.stack(init_obs2s, dim=0)
                acts = torch.stack(acts, dim=0)
                rews = torch.stack(rews, dim=0)
                ds = torch.stack(ds, dim=0)
                lps = torch.stack(lps, dim=0)

                initial_obs1.append(init_obs1s)
                initial_obs2.append(init_obs2s)
                initial_act.append(acts)
                initial_rewards.append(rews)
                initial_dones.append(ds)
                initial_logprob.append(lps)
    ...
    def _compute_retrace(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob):
        data = self._get_retrace(dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob)
        obs, initial_obs, initial_obs2, initial_act, initial_rewards, initial_dones, initial_logprob = data

        with torch.no_grad():
            rollout = self._rollout(network, obs[:, -1])
            states, actions, act_log_probs, noise_log_probs, total_log_probs, rewards, dones, next_obs, next_action, next_act_log_prob, next_noise, next_noise_log_prob = rollout

        states = torch.stack(states, dim=1)
        actions = torch.stack(actions, dim=1)
        rewards = torch.stack(rewards, dim=1).unsqueeze(dim=-1)
        act_log_probs = torch.stack(act_log_probs, dim=1).unsqueeze(dim=-1)
        dones = (torch.stack(dones, dim=1) > 0.5).unsqueeze(dim=-1).float()

        initial_obs = torch.cat([initial_obs, states], dim=1)
        initial_act = torch.cat([initial_act, actions], dim=1)
        initial_rewards = torch.cat([initial_rewards, rewards], dim=1)
        initial_logprob = torch.cat([initial_logprob, act_log_probs], dim=1)
        initial_dones = torch.cat([initial_dones, dones], dim=1)

        qrets = []
        q1s, q2s = [], []
        qret1, qret2 = self.critic_target(next_obs, next_action)
        qret = torch.min(qret1, qret2) - self.alpha.detach() * next_act_log_prob.unsqueeze(dim=-1)

        for s in range(self.k + self.horizon - 1, -1, -1):
            r = initial_rewards[:, s]
            d = initial_dones[:, s]
            o1 = initial_obs[:, s]
            a1 = initial_act[:, s]

            act_dist, _ = self.actor(o1)
            task_logprob = act_dist.log_prob(a1).sum(-1, keepdim=True)
            w = torch.exp(task_logprob - initial_logprob[:, s])
            w = torch.nan_to_num(w, nan=0.0, posinf=1.0, neginf=0.0)
            c = torch.min(torch.ones_like(task_logprob), w)

            q_i1, q_i2 = self.critic_target(o1, a1)
            q_i = torch.min(q_i1, q_i2)

            a = act_dist.sample()
            ent = act_dist.log_prob(a).sum(-1, keepdim=True)
            V1, V2 = self.critic_target(o1, a)
            V = torch.min(V1, V2)
            V = V - self.alpha.detach() * ent

            qret = r + self.gamma * qret * (1.0 - d)

            cq_i1, cq_i2 = self.critic(o1, a1)
            q1s.append(cq_i1)
            q2s.append(cq_i2)
            qrets.append(qret)
        ...
        qrets = torch.stack(qrets, dim=1).detach()
            
        err1 = (q1s - qrets).pow(2)
        err2 = (q2s - qrets).pow(2)
        critic_loss = torch.mean(err1) + torch.mean(err2)
    ...
    def _solve(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob, verbose=True):
        for epoch in range(self.epochs):
            retrace_loss = self._compute_retrace(network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, dataset_logprob)
            ...
            if (epoch + 1) % self.target_update_frequency == 0:
                ...
                if verbose:
                    print(f"Iteration {epoch} ; Actor value = {-actor_loss} ; Retrace loss = {retrace_loss} ; k = {self.k}")
                    sys.stdout.flush()
