...
from mbopg.mbrl_parallel1 import MBRL_solver
...
parser.add_argument('--surrogate-replay-capacity', type=int, default=int(1e5), help='Capacity of surrogate replay buffer')
...
mbrl_solver = MBRL_solver(obs_dim=obs_dim, action_dim=action_dim, horizon=args.plan_steps, epochs=args.mdp_epochs,
              ...
              actor_lr=args.actor_lr, critic_lr=args.critic_lr, capacity=args.replay_capacity, surrogate_capacity=args.surrogate_replay_capacity,
...
def add_to_dataset(current_state, u, reward, done, next_state, current_step, episode_steps):
    global args
    ...
    done_no_max = done
    if episode_steps == args.n_explore_steps:
        done_no_max = True
...
if rank == 0:
    ...
    while episode_steps < args.n_explore_steps:
        ...
        add_to_dataset(current_state, greedy_u, rew, done, next_state, current_steps, episode_steps)
