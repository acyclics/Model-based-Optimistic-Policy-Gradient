    ...
    def _solve_once(self, network, dataset_states, epsilon_greedy=False):
        ...
        for n_itr in range(self.actor_iterations_per_epoch):
            ...
            for t in range(self.horizon):
                ...
                if epsilon_greedy:
                    if np.random.uniform() < self.epsilon:
                        action = 2.0 * torch.rand((1, self.action_dim), device=self.device).float() - 1.0
                    else:
                        action = dist.rsample()
                else:
                    action = dist.rsample()
            ...
            for t1 in reversed(range(0, len(states))):
                ...
                V = torch.min(Q1, Q2)
                V = V[0] - self.alpha.detach() * log_probs[t1]
    ...
    def fill_replay(self, network, dataset_states):
        ...
        with torch.no_grad():
            for _ in range(warmup_epochs):
                _, _, obses, actions, rewards, next_obses, dones = self._solve_once(network, dataset_states, epsilon_greedy=True)
