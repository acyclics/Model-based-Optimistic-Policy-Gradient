...
def train_network(network):
    ...
    last_state = dataset_state[len(dataset_state) - args.n_explore_steps:]
    last_action = dataset_action[len(dataset_state) - args.n_explore_steps:]
    last_reward = dataset_rewards[len(dataset_state) - args.n_explore_steps:]
    last_done = dataset_dones[len(dataset_state) - args.n_explore_steps:]
    last_next_state = dataset_next_states[len(dataset_state) - args.n_explore_steps:]

    dataset = list(zip(dataset_state, dataset_action, dataset_rewards, dataset_dones, dataset_next_states))

    while avg_llh < args.llh_threshold or epoch < args.network_epochs or avg_new_llh < args.llh_threshold:
        avg_llh = 0
        avg_loss = 0
        avg_H = 0
        avg_err = 0
        ...
        for idx in range(0, len(last_state), minibatch_size):
            mb_s = last_state[idx : idx + minibatch_size]
            mb_a = last_action[idx : idx + minibatch_size]
            mb_r = last_reward[idx : idx + minibatch_size]
            mb_d = last_done[idx : idx + minibatch_size]
            mb_ns = last_next_state[idx : idx + minibatch_size]
            ...
            log_lik = network.eval(mb_s, mb_a, mb_r, mb_d, mb_ns)
        ...
        avg_new_llh /= (len(last_state) / minibatch_size)
        avg_loss /= (len(dataset_state) / minibatch_size)
        avg_H /= (len(dataset_state) / minibatch_size)
        avg_err /= (len(dataset_state) / minibatch_size)

        print(f"Epoch = {epoch} ; Average loss = {avg_loss} ; Likelihood = {avg_llh} ; Avg new LLH = {avg_new_llh} ; H = {avg_H} ; Err = {avg_err} ; Beta = {network.beta}")
        sys.stdout.flush()
...
for loop_n in range(N_OVERALL_LOOPS):
    if (loop_n + 1) % args.clear_steps == 0:
        ...
        policy, critic = mbrl_solver.solve(network, dataset_state, reset=False, verbose=True)
