...
class MBRL_solver(nn.Module):
    ...
    def _solve_once(self, network, dataset_states, epsilon_greedy=False):
        ...
        for t in range(self.horizon):
            ...
            self.replay_buffer.add(obs.detach(),
                                       ...
                                       next_obs.detach(), 
                                       done.unsqueeze(dim=-1).float().detach(), 
                                       False)
    ...
    def solve_surrogate(self, network):
        ...
        for idx in range(len(self.surrogate_replay_buffer)):
            initial_obs = [self.surrogate_replay_buffer.obses[idx] for _ in range(self.actor_repeat_per_epoch)]
            initial_obs = np.asarray(initial_obs)
            #obs = torch.stack(initial_obs, dim=0)
            obs = torch.from_numpy(initial_obs).to(self.device)
            ...
            for t in range(self.horizon):
                ...
                for jdx in range(self.actor_repeat_per_epoch):
                    aug_replay_buffer.add(obs[jdx].detach().cpu(),
                                            action[jdx, 0:-self.noise_dim].detach().cpu(), 
                                            reward[jdx].detach().cpu(), 
                                            next_obs[jdx].detach().cpu(), 
                                            done[jdx] > 0.5, 
                                            False)
        ...
        for epoch in range(max_epochs):
            obs, action, reward, next_obs, not_done, _ = aug_replay_buffer.sample(self.critic_batch_size)
            ...
            if epoch % 2 == 0:
                utils.soft_update_params(self.surrogate_critic, self.surrogate_critic_target, 0.005)

                obs, action, reward, next_obs, not_done, _ = self.surrogate_replay_buffer.sample(self.critic_batch_size)
