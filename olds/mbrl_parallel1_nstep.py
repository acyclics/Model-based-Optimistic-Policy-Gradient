...
from mbopg.memory.base import LazyMultiStepMemory
...
class MBRL_solver(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, horizon, epochs, actor_iterations_per_epoch, actor_repeat_per_epoch,
        ...
        self.surrogate_replay_buffer = LazyMultiStepMemory(int(self.capacity), self.obs_dim, self.action_dim - noise_dim, device, self.gamma, self.multi_steps)
    ...
    def gather_surrogate(self, network, dataset_states):
        with torch.no_grad():
            ...
            for t in reversed(range(self.horizon)):
                nstep_rew = no_ret_rewards[t] + self.gamma * (1 - dones[t]) * nstep_rew
                nstep_rews.append(nstep_rew)
            rewards.extend(reversed(nstep_rews))
    ...
    def solve_surrogate(self, augment=True):
        ...
        state = np.empty((len(self.surrogate_replay_buffer['state']), self.obs_dim), dtype=np.float32)
        next_state = np.empty((len(self.surrogate_replay_buffer['next_state']), self.obs_dim), dtype=np.float32)

        for idx in range(len(self.surrogate_replay_buffer['state'])):
            state[idx, ...] = self.surrogate_replay_buffer['state'][idx]
            next_state[idx, ...] = self.surrogate_replay_buffer['next_state'][idx]

        state = torch.FloatTensor(state).to(self.device)
        next_state = torch.FloatTensor(next_state).to(self.device)
        action = torch.FloatTensor(self.surrogate_replay_buffer['action'][0:len(self.surrogate_replay_buffer['state'])]).to(self.device)
        reward = torch.FloatTensor(self.surrogate_replay_buffer['reward'][0:len(self.surrogate_replay_buffer['state'])]).to(self.device)
        done = torch.FloatTensor(self.surrogate_replay_buffer['done'][0:len(self.surrogate_replay_buffer['state'])]).to(self.device)
