...
class MBRL_solver(nn.Module):
    ...
    def _solve_once(self, network, dataset_states, epsilon_greedy=False):
        ...
        target_V = target_V[:, 0] - self.alpha.detach() * log_probs[-1]

        for t in reversed(range(len(rewards))):
            target_V = rewards[t] - self.alpha.detach() * log_probs[t] + (1.0 - dones[t]) * self.gamma * target_V
            target_V = target_V.detach()

            current_Q1, current_Q2 = self.critic(states[t], actions[t])
            critic_loss = (current_Q1 - target_V) ** 2 + (current_Q2 - target_V) ** 2
            critic_losses.append(critic_loss)
        
        critic_losses = torch.stack(critic_losses, dim=0)
        critic_losses = torch.mean(critic_losses, dim=0)

        return actor_values, critic_losses, total_log_probs
    ...
    def _solve(self, network, dataset_states, verbose=True):
        for epoch in range(self.epochs):
            ...
            actor_values, critic_losses, total_log_probs = self._solve_once(network, dataset_states)
    ...
    def fill_replay(self, network, dataset_states):
        ...
        with torch.no_grad():
            for _ in range(self.critic_warmup_epochs):
                _, _, _ = self._solve_once(network, dataset_states, epsilon_greedy=True)
    ...
    def collect_rollouts(self, policy, critic, network, dataset_states):
        ...
        starting_idx = self.replay_buffer.idx
        actor_values, _, total_log_probs = self._solve_once(network, dataset_states)
        final_idx = self.replay_buffer.idx
        if starting_idx > final_idx:
            final_idx = self.replay_buffer.capacity
