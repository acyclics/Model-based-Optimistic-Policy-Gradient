...
class MBRL_solver(nn.Module):
    ...
    def solve(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, reset=True, verbose=True):
        if reset:
            self.full_reset()

        self._solve(network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, verbose=verbose)
    ...
    def _solve_once_critic(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones):
        ...
        target_Q1, target_Q2 = self.critic_target(next_obs, next_action[:, 0:-self.noise_dim])
        ...
        return_expansion = initial_rewards + self.gamma * (1 - initial_dones) * mean_return_expansion
        return_expansion = return_expansion.detach()
        
        current_Q1, current_Q2 = self.critic(initial_obs, initial_act)
        critic_loss = torch.mean((current_Q1 - return_expansion).pow(2)) + torch.mean((current_Q2 - return_expansion).pow(2))
        ...
        states = torch.cat([initial_obs, states], dim=0)
        states = states.detach()

        return critic_loss, states
    ...
    def _solve(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones, verbose=True):
        for epoch in range(self.epochs):
            critic_loss, obs = self._solve_once_critic(network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones)

            if (epoch + 1) % self.surrogate_target_update_frequency == 0:
                utils.soft_update_params(self.critic, self.critic_target, self.tau)
                actor_loss = -self._solve_once_actor(network, dataset_states).mean()

                # optimize the actor
                self.manual_set_zero_grads()
                actor_loss.backward()
