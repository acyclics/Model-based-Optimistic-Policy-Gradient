...
from mbopg.mbrl_on import MBRL_solver
...
def add_to_dataset(current_state, u, reward, done, next_state, logprob, current_step):
    global args
    ...
    lp = torch.from_numpy(logprob.astype(np.float32)).to(DEVICE)
...
while episode_steps < args.n_explore_steps:
    action, greedy_u, logprob = policy.act(current_state, sample=True)
    ...
    add_to_dataset(current_state, action, rew, done, next_state, logprob, current_steps)
...
for loop_n in range(N_OVERALL_LOOPS):
    policy, critic = mbrl_solver.solve(network, dataset_state, dataset_action, dataset_rewards, dataset_next_states, dataset_logprob, dataset_dones, verbose=True)
