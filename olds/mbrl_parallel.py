...
class MBRL_solver(nn.Module):
    ...
    def _solve_once(self, network, dataset_states):
        ...
        return actor_values, total_log_probs, states, actions, rewards, next_states, dones, done_no_maxs

    def _solve(self, network, dataset_states, verbose=True):
        for epoch in range(self.epochs):
            actor_values, total_log_probs, states, actions, rewards, next_states, dones, done_no_maxs = self._solve_once(network, dataset_states)
    ...
    def solve_surrogate(self, augment=True):
        ...
        while epoch < self.surrogate_epochs or continue1:
            ...
            if self.comm.Iprobe(source=0, tag=5) and augment and continue1:
                ...
                if continue1:
                    with torch.no_grad():
                        for jdx in range(self.actor_iterations_per_epoch * self.actor_repeat_per_epoch):
                            for t in range(self.horizon):
                                """
                                aug_replay_buffer.append(traj['states'][t][jdx].cpu(),
                                                        traj['actions'][t][jdx][0:-self.noise_dim].cpu(), 
                                                        traj['rewards'][t][jdx].cpu(), 
                                                        traj['next_states'][t][jdx].cpu(), 
                                                        traj['dones'][t][jdx].cpu(), 
                                                        traj['done_no_maxs'][t][jdx].cpu())
                                """
                                done = (traj['dones'][t][jdx].cpu() > 0.5).float()
                                done_no_max = (traj['done_no_maxs'][t][jdx].cpu() > 0.5).float()
                                aug_replay_buffer.add(traj['states'][t][jdx].cpu(),
                                                        traj['actions'][t][jdx][0:-self.noise_dim].cpu(), 
                                                        traj['rewards'][t][jdx].cpu(), 
                                                        traj['next_states'][t][jdx].cpu(), 
                                                        done, 
