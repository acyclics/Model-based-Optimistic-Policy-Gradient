...
class MBRL_solver(nn.Module):
    ...
    def _solve_once_critic(self, network, dataset_states, dataset_actions, dataset_rewards, dataset_next_states, dataset_dones):
        ...
        sampled_states1,
                          sampled_states2, sampled_actions, sampled_rewards, sampled_dones,
                          sampled_logprob

        return critic_loss

    def _solve_once_actor(self, network, dataset_states, dataset_actions, dataset_rewards,
                          dataset_next_states, dataset_dones, dataset_logprob, sampled_states1,
                          sampled_states2, sampled_actions, sampled_rewards, sampled_dones,
                          sampled_logprob):
        total_log_probs = []
        initial_obs1 = []
        ...
        obs = torch.stack(initial_obs2, dim=0)
        obs = torch.cat([obs, sampled_states2], dim=0)

        initial_obs = torch.stack(initial_obs1, dim=0)
        initial_obs = torch.cat([initial_obs, sampled_states1], dim=0)

        initial_act = torch.stack(initial_act, dim=0)
        initial_act = torch.cat([initial_act, sampled_actions], dim=0)

        initial_rewards = torch.stack(initial_rewards, dim=0)
        initial_rewards = torch.cat([initial_rewards, sampled_rewards], dim=0)

        initial_dones = torch.stack(initial_dones, dim=0)
        initial_dones = torch.cat([initial_dones, sampled_dones], dim=0)

        initial_logprob = torch.stack(initial_logprob, dim=0)
        initial_logprob = torch.cat([initial_logprob, sampled_logprob], dim=0)
