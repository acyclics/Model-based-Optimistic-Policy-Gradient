...
class MBRL_solver(nn.Module):
    ...
    def solve(self, network, dataset_states, reset=True, verbose=True):
        ...
        self.aug_replay_buffer = ReplayBuffer(obs_shape=[self.obs_dim], action_shape=[self.action_dim - self.noise_dim], capacity=int(self.capacity), device=self.device)
        ...
        for idx in range(len(self.surrogate_replay_buffer)):
            ...
            self.aug_replay_buffer.add(state,
                                    action,
                                    reward,
                                    next_state,
                                    done,
