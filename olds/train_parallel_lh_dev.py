...
from mbopg.mbrl_parallel_lh import MBRL_solver
...
mbrl_solver = MBRL_solver(obs_dim=obs_dim, action_dim=action_dim, horizon=args.plan_steps, epochs=args.mdp_epochs,
              ...
              critic_batch_size=args.critic_batchsize,
...
def train_network(network):
    ...
    while avg_llh < args.llh_threshold or epoch < args.network_epochs:
        ...
        print(f"Second ; Epoch = {epoch} ; Average loss = {avg_loss} ; Likelihood = {avg_llh} ; H = {avg_H} ; Err = {avg_err} ; Beta = {network.beta}")
        sys.stdout.flush()
