...
class DoubleQCritic(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, noise_dim, hidden_dim, hidden_depth):
        ...
        self.Q1_shared = utils.mlp(obs_dim + action_dim - noise_dim, hidden_dim, hidden_dim, hidden_depth)
        self.Q1 = utils.mlp(hidden_dim, hidden_dim, 1, 2)
        self.Q1_res = utils.mlp(hidden_dim + noise_dim, hidden_dim, 1, 2)

        self.Q2_shared = utils.mlp(obs_dim + action_dim - noise_dim, hidden_dim, hidden_dim, hidden_depth)
        self.Q2 = utils.mlp(hidden_dim, hidden_dim, 1, 2)
        self.Q2_res = utils.mlp(hidden_dim + noise_dim, hidden_dim, 1, 2)

        self.outputs = dict()
        self.apply(utils.weight_init)

    def mean_forward(self, obs, action):
        assert obs.size(0) == action.size(0)

        obs_action = torch.cat([obs, action], dim=-1)

        q1 = self.Q1_shared(obs_action)
        q1 = self.Q1(q1)

        q2 = self.Q2_shared(obs_action)
        q2 = self.Q2(q2)
    ...
    def forward(self, obs, action):
        ...
        obs_action = torch.cat([obs, action], dim=-1)
        obs_action_no_noise = obs_action[..., 0:-self.noise_dim]
        noise = action[..., -self.noise_dim:]

        q1_shared = self.Q1_shared(obs_action_no_noise)
        q1_res = torch.cat([q1_shared, noise], dim=-1)
        q1 = self.Q1(q1_shared) + self.Q1_res(q1_res)
        
        q2_shared = self.Q2_shared(obs_action_no_noise)
        q2_res = torch.cat([q2_shared, noise], dim=-1)
        q2 = self.Q2(q2_shared) + self.Q2_res(q2_res)
...
class MBRL_solver(nn.Module):
    ...
    def update_surrogate_critic(self):
        obs2, action2, reward2, next_obs2, not_done2, _ = self.surrogate_replay_buffer.sample(self.critic_batch_size)

        #dist = self.actor_target(next_obs)
        dist2 = self.actor(next_obs2)

        next_action2 = dist2.sample()
        log_prob2 = dist2.log_prob(next_action2).sum(-1, keepdim=True)

        target_Q12, target_Q22 = self.critic_target(next_obs2, next_action2)
        target_V2 = torch.min(target_Q12, target_Q22) - self.alpha.detach() * log_prob2
        target_Q2 = reward2 + (not_done2 * self.gamma * target_V2)
        target_Q2 = target_Q2.detach()

        # get current Q estimates
        current_Q12, current_Q22 = self.critic.mean_forward(obs2, action2)
        critic_loss2 = F.mse_loss(current_Q12, target_Q2) + F.mse_loss(current_Q22, target_Q2)
    ...
    def _solve(self, network, dataset_states, verbose=True):
        for epoch in range(self.epochs):
            ...
            for _ in range(self.critic_min_iterations_per_epoch // 2):
                critic_losses += self.update_critic()
            
            for _ in range(self.critic_min_iterations_per_epoch // 2):
                critic_losses += self.update_surrogate_critic()
            
            critic_losses /= self.critic_min_iterations_per_epoch
